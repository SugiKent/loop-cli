...
// ToggleTodo は issue の stage:todo を 1 操作 1 ラベルで切り替える（不変条件 1）。
// 判定は ViewIssue で読み直した現在のラベルで行う。画面のラベルは最後の取得時点のもので、
// 続けて 2 回押したときの 2 回目が「もう一度付ける」になってしまう。
// added は付けたなら true、外したなら false。
func ToggleTodo(ctx context.Context, client gh.GHClient, repo string, number int) (bool, error) {
	...
	stages := model.IssueStages(labels)
	switch {
	case len(stages) >= 2:
		return false, fmt.Errorf("%w（%s）", ErrMultipleStages, strings.Join(stages, " "))
	case model.HasLabel(labels, model.LabelStageTodo):
		// 付いているものを外す操作は常に許す（blocked と同時に付いていても取り消しをできなくしない）。
		return false, client.RemoveLabel(ctx, repo, number, model.LabelStageTodo)
	case len(stages) == 1:
	...
	return true, client.AddLabel(ctx, repo, number, model.LabelStageTodo)
}
