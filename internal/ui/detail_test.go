...
func TestDetailFooters(t *testing.T) {
	m, _ := send(detailModel(150, 40, []model.Card{longBodyCard()}), enterKey)
	...
	lines = linesOf(m)
	footer = lines[len(lines)-1]
	for _, want := range []string{"Esc 戻る", "g issue へ", "a 回答", "L ラベル", "m merge", "c close", "n 新規", "o ブラウザ", "? ヘルプ", "u URL", "q 終了"} {
		if !strings.Contains(footer, want) {
...
func TestShortTerminalDropsPRList(t *testing.T) {
	...
	m, _ := send(detailModel(150, 7, []model.Card{issueCard(issue, prs, "読む")}), enterKey)
