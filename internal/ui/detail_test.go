...
func TestDetailFooters(t *testing.T) {
	m, _ := send(detailModel(140, 40, []model.Card{longBodyCard()}), enterKey)

	lines := linesOf(m)
	footer := lines[len(lines)-1]
	for _, want := range []string{"Esc 戻る", "Tab PR 選択", "x 展開", "a 回答", "t todo", "L ラベル", "m merge", "n 新規", "o ブラウザ", "? ヘルプ", "u URL", "q 終了"} {
	...
	lines = linesOf(m)
	footer = lines[len(lines)-1]
	for _, want := range []string{"Esc 戻る", "g issue へ", "a 回答", "L ラベル", "m merge", "n 新規", "o ブラウザ", "? ヘルプ", "u URL", "q 終了"} {
		if !strings.Contains(footer, want) {
...
func TestShortTerminalDropsPRList(t *testing.T) {
	...
	m, _ := send(detailModel(140, 7, []model.Card{issueCard(issue, prs, "読む")}), enterKey)
