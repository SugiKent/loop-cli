...
func coloredLabel(name, color string) string { return renderLabelName(name, color) }
...
// wantPlain は語が色を伴わずに現れることを確かめる。SGR は必ず `m` で閉じてから語が続くので、
// 「`m` + 語」が無ければその語にスタイルは適用されていない。
func wantPlain(t *testing.T, view, word, what string) {
	t.Helper()
	if !strings.Contains(view, word) {
		t.Fatalf("%s が画面に無い: %q", what, word)
	}
	if strings.Contains(view, "m"+word) {
...
func TestPreviewLabelsAreColoredAndTableRowKeepsKindColor(t *testing.T) {
	...
	wantIn(t, view, coloredLabel("propose", "0e8a16"), "プレビューの propose")
	wantIn(t, view, coloredLabel("question", "d876e3"), "プレビューの question")
...
func TestLabelColorsAreReplacedOnEachFetch(t *testing.T) {
	...
	wantIn(t, m.View().Content, coloredLabel("stage:propose", "0e8a16"), "1 回目の取得の色")

	m, _ = send(m, fetchedMsg{res: &fetch.Result{Cards: cards, LabelColors: proposeColors("1d76db")}, at: at})
	wantIn(t, m.View().Content, coloredLabel("stage:propose", "1d76db"), "2 回目の取得の色")
}
...
func TestLabelColorsSurviveFetchFailure(t *testing.T) {
	...
	wantIn(t, m.View().Content, coloredLabel("stage:propose", "0e8a16"), "取得失敗の後の色")
}
...
func TestPRListRowLabelsAndStatesAreColored(t *testing.T) {
	...
	wantIn(t, view, coloredLabel("propose", "0e8a16"), "PR 一覧行の段階ラベル")
	wantIn(t, view, coloredLabel("question", "d876e3"), "PR 一覧行の question")
...
func TestCardHeaderStageAndBadgesAreColored(t *testing.T) {
	...
	wantIn(t, view, "段階: "+coloredLabel("stage:propose", "0e8a16"), "ヘッダの段階ラベル")
	wantIn(t, view, "["+coloredLabel("question", "d876e3")+"]", "ヘッダの question の badge")
}

func TestCheckRowStatesAreColored(t *testing.T) {
...
func TestFetchFailureWordsAreRed(t *testing.T) {
	issue := &model.Issue{Number: 108, Title: "手書き", UpdatedAt: at}
	pr := prOf(131, "OPEN", []string{"propose"}) // MergeState は nil（取得失敗）
	...
	if want := coloredState("取得失敗"); !strings.Contains(want, "38;2;248;81;73") {
		t.Errorf("取得失敗 が暗い端末の赤 #F85149 でない: %q", want)
	}
}
...
func TestMergeConfirmLabelsAndStatesAreColored(t *testing.T) {
	...
	wantIn(t, view, coloredLabel("propose", "0e8a16"), "確認画面のラベル")
...
func TestCloseConfirmLabelsAreColored(t *testing.T) {
	...
	wantIn(t, view, coloredLabel("propose", "0e8a16"), "close の確認画面のラベル")
	wantIn(t, view, coloredLabel("question", "d876e3"), "close の確認画面の question")
...
func TestLabelListNamesUseTheirOwnColor(t *testing.T) {
	...
	wantIn(t, view, coloredLabel("docs", "0075ca"), "一覧の docs")
	wantIn(t, view, coloredLabel("wip", "fbca04"), "一覧の wip")
