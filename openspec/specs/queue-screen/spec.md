...
- mvp.md の表にある他のキー `A` / `s` / `g` / `x` / `/` / `v` / `h` / `l` / `←` / `→` と、表に無いキー（`p` と `Esc` を含む）は、キュー画面では `Model` を変えず、コマンドも返さない。担当は `s` が後続 change（s15 の枠のうち `n` だけをこの change が実装した）、`A` が s16、`v` / `h` / `l` / `←` / `→` が s17、`/` が s19。`g` / `Esc` と、mvp.md の表に無い `G` / `Home` / `End` は詳細画面のキーであり s09 `card-detail` が定める（キュー画面では何もしない）。`x` は s38 より前は詳細画面のキーだったが、コメントの折りたたみが廃止されたので、どの画面でも何もしないキーになった（`card-detail`「コメントは AI も人も常に全文を出す」）。`G` / `Home` / `End` は s37 が `card-detail`「詳細の本文領域はスクロールし、ヘッダ領域は固定する」へ足したもので、詳細画面の本文領域を末尾・先頭へ送る。キュー画面の表は選択行に追従する窓を持たないので、この 3 つをキュー画面では動かさない。`p` は s21 まで表とプレビューの切替だったが、2 ペインが常設になったため何もしないキーに戻った。`u` は s22 が、`L` は s28 が、`c` は s26 が mvp.md キーバインド表に足すキーで、どれも上の箇条書きのとおりキュー画面で動く（`c` は s26 より前は表に無いキーとして何もしなかった）。小文字の `l` は s17（カンバンの列移動）の予約のままで、キュー画面では何もしない
