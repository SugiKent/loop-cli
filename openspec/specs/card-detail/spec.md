...
`internal/ui` の `Model` は画面の状態として キュー / カード詳細 / PR 詳細 の 3 つを MUST 持ち、初期状態はキューである。`Update` はキー入力を画面の状態ごとに次のとおり扱う。
...
フッタの左はその画面で動くキーだけのヒント（カード詳細: `Esc 戻る  Tab PR 選択  Enter PR を開く  x 展開  g PR へ  j/k スクロール  q 終了`。`PRs` が空なら `Tab PR 選択  Enter PR を開く  g PR へ` を省く。PR 詳細: `Esc 戻る  x 展開  g issue へ  j/k スクロール  q 終了`）、右は s08 と同じステータス（スピナー / エラー）とする。
...
- **THEN** 1 回目の最終行に `Esc 戻る`、`Tab PR 選択`、`x 展開`、`q 終了` が含まれ `1-4/Tab タブ` は含まれない。2 回目の最終行に `Esc 戻る` と `g issue へ` が含まれ `Tab PR 選択` は含まれない
