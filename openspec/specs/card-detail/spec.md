...
フッタの左はその画面で動くキーだけのヒント（カード詳細: `Esc 戻る  Tab PR 選択  Enter PR を開く  x 展開  g PR へ  a 回答  j/k スクロール  q 終了`。`PRs` が空なら `Tab PR 選択  Enter PR を開く  g PR へ` を省く。PR 詳細: `Esc 戻る  x 展開  g issue へ  a 回答  j/k スクロール  q 終了`）、右は s08 のステータスと s10 の回答ステータスとする。
...
- **THEN** 1 回目の最終行に `Esc 戻る`、`Tab PR 選択`、`x 展開`、`a 回答`、`q 終了` が含まれ `1-4/Tab タブ` は含まれない。2 回目の最終行に `Esc 戻る` と `g issue へ` と `a 回答` が含まれ `Tab PR 選択` は含まれない

#### Scenario: PR の無いカードのフッタには PR のキーを出さない
- **WHEN** `example` の issue 140 の Card（`PRs` 空）の詳細を開き、`View` から ANSI エスケープを除いて読む
- **THEN** 最終行に `Esc 戻る` と `x 展開` が含まれ、`Tab PR 選択`、`Enter PR を開く`、`g PR へ` は含まれない
