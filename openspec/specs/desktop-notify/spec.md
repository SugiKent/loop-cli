...
`internal/ui` は通知関数の型 `Notifier`（`func(title, body string) error`）を MUST 公開し、`Model` は `New` の `Options.Notify` で受け取る。`cmd/sugi-loop` は `Config.Notify` が true のとき `beeep.Notify(title, body, "")`（s06 `notify test` と同じ呼び方。icon は空文字列。`nil` を渡すと beeep v0.11.2 はエラーになる）を呼ぶ関数を渡し、false のとき `nil` を渡す。`nil` なら通知しない。
...
- 増えた Card 1 枚につき `Notifier` を 1 回、`addedNow` の並び順で呼ぶ。タイトルは `sugi-loop`、本文は 1 行目に `Card.Result.Summary`、2 行目に主体の表示名（s10 と同じ `<Repo> PR#<n>` / `<Repo> #<n>`）を改行で結んだ 2 行
...
- **THEN** 記録は 1 件で、`title` は `sugi-loop`、`body` は 1 行目が今やるタブの行の `Summary` と同じ文字列、2 行目が `org/app PR#131` である
