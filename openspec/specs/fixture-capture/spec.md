...
`loop-cli-dev fixture capture --repo <owner/name> --alias <alias>` は、稼働中リポジトリ 1 件の open issue / open PR と、その全件の詳細を採取し、伏せ字にして `internal/gh/testdata/fixtures/<alias>/` に s03 `gh-fake` の命名規則で MUST 保存する。
