...
- PR 詳細画面: 詳細の対象の PR
- close の確認画面・merge の確認画面・回答の確認画面・作成の確認画面・ヘルプ画面・URL 一覧画面・ラベル一覧画面: 何もしない

`c` のヒントは `?` のヘルプ（`help-screen`「ヘルプ画面は実装済みのキーだけを一覧する」）と、3 画面のフッタの左に
`c close` として出す。位置は `m merge` の次（`m merge` を出さない `PRs` が空のカード詳細では `L ラベル` の次で、3 画面で同じ並びになる）で、文字列は `queue-screen`
...
- **WHEN** Scenario「propose PR の確認画面には却下の注意が出る」の `Model` に `a`、`t`、`m`、`c`、`o`、`?`、`u`、`L`、`n` を 1 つずつ与える
...
- close 中: フッタの右側に `<表示名> を close 中` を出す。この間の `t` / `a` / `m` / `c` / `n` は何もしないが、書き込みではない
