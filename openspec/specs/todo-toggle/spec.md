...
`Model` は書き込み（s11 のラベル切り替え、s10 のコメント投稿、s14 の merge、s15 の issue 作成、s26 の close）のコマンドを返してから結果のメッセージを受け取るまでを「書き込み中」として MUST 扱い、その間の `t` と `a` と `m` と `n` と `c` はどの画面でも何もしない（コマンドを返さず、`Model` を変えない）。s14 の `m` が押されてから merge の判断材料を取り直している間（まだ書き込んでいない）も同じ扱いにし、merge の手続きを重ねて始めさせない（s14 `merge-pr`「m は画面の対象 PR を決めて GitHub から状態を取り直す」）。s26 の `c` は押下時に `gh` を呼ばないので、確認画面へ移るところは書き込み中に入らず、`y` を押して close のコマンドが返ってからが書き込み中である（s26 `close-issue-pr`「c は画面の対象を決めて close の確認画面を開く」）。s10「前の投稿の結果を待っている間は `a` を押しても何もしない」を、ラベル切り替え中と merge の手続き中と issue の作成中と close 中にも広げたものである（s10 の Requirement の範囲を狭めない）。同じ対象への二重の書き込みを防ぐためであり、対象ごとの排他は持たない。書き込み中のフラグは s10 の投稿中フラグと 1 つで共有してよい（design.md）。
エディタが開いている間と確認画面を読んでいる間はまだ書き込んでいないので、この Requirement の「書き込み中」に当たらない（エディタが開いている間はキー入力が `Model` に届かず、確認画面のキーは s10 / s14 / s15 / s26 の各確認画面が定める）。

#### Scenario: 切り替え中の t と a と m は効かない
- **WHEN** 今やるタブの issue 108 の Card（主体は PR 131）を選んだ `Model` に `t` を与えた直後（返ったコマンドを実行する前）に、もう一度 `t` を与え、続けて `a` と `m` と `c` を与える
- **THEN** どれもコマンドは返らず、`Fake.Calls` は空のまま（最初の `t` のコマンドをまだ実行していないため）で、スタブ `Editor` は呼ばれない

#### Scenario: 切り替え中の n は効かない
- **WHEN** 同じ手順（`t` を与えてコマンドが返った直後）で `n` を与える
- **THEN** コマンドは返らず、`Fake.Calls` は空のままで、スタブ `Editor` は呼ばれない
...
#### Scenario: issue 作成中の t と a と m は効かない
- **WHEN** s15 の手順（固定文字列 `タイトル\n\n本文` を返すスタブ `Editor` で `n` → 編集完了のメッセージを `Update` に渡して作成の確認画面に移り、`y` を与えて作成のコマンドが返った直後）で `t` と `a` と `m` を与える
- **THEN** どれもコマンドは返らず、`Fake.Calls` に `AddLabel` / `RemoveLabel` / `CommentPR` / `CreateIssue` は無い

#### Scenario: close 中の t と a は効かない
- **WHEN** s26 の手順（今やるタブで `c` → 確認画面で `y` を与えて close のコマンドが返った直後）で `t` と `a` を与える
- **THEN** どちらもコマンドは返らず、`Fake.Calls` に `AddLabel` / `RemoveLabel` / `CommentPR` は無い
