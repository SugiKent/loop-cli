...
- `label-picker`: `L` のラベルの表を、取得が成功するたびに捨てる（未確定の判断 Q1 の回答による）
...
- 人から見た変化: `config.yml` から `mode` を消す（消さないと起動しない。未確定の判断 Q3）。ILD のリポジトリで `t` が
...
### Q1. 取得したラベル一覧を `L`（ラベル一覧画面）の表にも流し込むか

`L` は `internal/ui/labels.go` に「リポジトリ名 → ラベル一覧」の表を持ち、表に無ければ `ListLabels` を呼んで入れ、
**プロセスが終わるまで捨てない**（`label-picker`「表は `R` でも自動更新でも捨てない」）。この change で `Fetch` が
毎回すべてのリポジトリのラベル一覧を取るので、放置すると同じデータを 2 経路で取り、`L` の一覧だけが古いまま残る。

- 選択肢 A（推奨）: **取得が成功するたびに `L` の表を捨てる。** 次に `L` を押したときに取り直すので、GitHub 側でラベルを
  作れば次の更新のあとの `L` に出る。`label-picker` の Requirement 1 本を MODIFIED して 1 段落を書き換え、Scenario を 2 本足す。
  `gh` の呼び出しは、更新のあとに `L` を押したときだけ 1 回増える
- 選択肢 B: `Result` にラベル一覧そのもの（`Labels map[string][]gh.RepoLabel`）を入れ、取得のたびに `L` の表を差し替える。
  `gh` の呼び出しは増えず `L` は常に即座に開くが、`label-picker` の Scenario の多くが「取得の後は取りに行かない」形に変わり、
  10 本前後の書き直しが要る
- 選択肢 C: 何もしない（issue #24 の記述どおり）。`L` の一覧は再起動まで古いままで、同じラベル一覧を 2 経路で取る状態が残る
- 依存: なし

### Q2. `stage:todo` も `To Do` も持たないリポジトリをどう扱うか

- 選択肢 A（推奨）: **方式を「判定できない」とし、`t` を拒否する。** フッタに赤で
  `<Repo> の運用方式が分かりません（stage:todo / To Do のラベルがありません）` を出す。0 件ヒントの 2 行目も
  `issue-driven-sdd の routines-setup を回すか、issue-label-driven の To Do ラベルを作ってください` に直す
- 選択肢 B（issue #24 の既定）: **`label` に倒す。** `t` は `To Do` を書きに行き、`gh` が「そんなラベルは無い」で失敗して、
  その文字列がフッタに赤で出る。人から見ると理由は分かるが、`t` を押すまで分からず、失敗の文言は `gh` 任せになる
- 依存: なし（回答は 0 件ヒントの文言も決める）

### Q3. 既存の `config.yml` に残っている `mode:` キーをどう扱うか

`repos` の要素は未知のキーがあると起動エラーになる（`internal/config/config.go:71`）。`mode` を消すと、いま `mode: label` を
書いている設定ファイルは**そのままでは起動しなくなる**。

- 選択肢 A（推奨）: 専用のエラー文で止める。`repos[<i>] "<repo>": mode は廃止しました。ラベル一覧から自動で判定するので、
  この行を削除してください` を返す。`config.go` に `case "mode":` を 1 本残す
- 選択肢 B: 汎用の未知キーのエラーに任せる（`repos の要素に未知のキー "mode" があります`）。コードは減るが、なぜ消えたのかは分からない
- 選択肢 C: `mode` を読み捨てて起動する。設定ファイルを直さなくても動くが、書いた値が黙って無視される
- 依存: なし

### Q4. `label` 方式の行 D からも `Closes #n` の絞り込みを外すか

行 C から外すことは issue #24 が決めている。行 D（未 resolve の review thread の最終コメントが AI）は今も `Closes #n` を条件にしている。

- 選択肢 A（推奨・issue #24 の既定）: **外す。** 行 C とそろい、`Closes #n` を書いていない PR でも未 resolve のレビュー質問が
  「質問」（優先度 1）として出る。外部からの PR に AI のレビューが付いた場合もここに出る
- 選択肢 B: 残す。`Closes #n` の無い PR は、レビュー質問が残っていても「その他」（優先度 6）に出る。同じ `[1]今やる` タブの
  中で並ぶ位置だけが変わる
- 依存: なし
