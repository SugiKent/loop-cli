...
- `Space` で選んだ行の印を切り替え、`Enter` で送信する（回答 Q6）。**送信は変更予定をまとめて 1 回の `gh` 呼び出しで反映する。** 複数のラベルを付け外ししても書き込みは 1 回である（回答 Q4）
- `Esc` は変更予定を捨てて元の画面に戻る。変更予定が 0 件のまま送信すると、何も書かずに閉じてフッタに `<repo> #<n> のラベルに変更はありません` を出す（文言は `label-picker` spec が正本）
...
- **フッタのキーヒント 4 つすべてに `L ラベル` を足す（Q5: A）。** キュー画面は 80 → 90 列、PR 詳細は 82 → 92 列、カード詳細は 117 → 127 列（PR 無しは 69 → 79 列で収まる）。`queue-screen` spec:150 と `card-detail` spec:194 が既に 2 回記録している「キーを隠すよりキーを出すことを採った」を続ける
- **一覧画面では `Space` で印を切り替え、`Enter` で送信する（Q6: A）。** 行の移動は `j` / `k` / `↑` / `↓`、破棄は `Esc`
...
- **ラベルの一覧に色を出さない。** `gh label list` は `color` も返すが、種別の色（`internal/ui/view.go` の `kindStyle`）と意味の違う色が同じ画面に並ぶので使わない
