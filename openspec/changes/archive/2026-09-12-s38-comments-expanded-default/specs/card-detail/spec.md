...
**Reason**: 詳細画面を開いた人が最初に読みたいのは、AI が書いた問い（`blocked-by: human` の選択肢、
PR リスク評価、grill の質問）そのものである。それを既定で隠し、`x` を押して初めて読める形は、
「人の出番だけを並べて捌く」という loop-cli の目的に対して 1 打鍵ぶん遠い（issue #60）。
折りたたみそのものを廃止するので、この Requirement が定める 2 つの表示（折りたたみ時と展開時）と
`x` の切り替えは、Requirement「コメントは AI も人も常に全文を出す」が 1 つの表示に置き換える。
**Migration**: `▌AI  HH:MM` の見出しと `▌` 付きの全行、人のコメントの `<Author>  HH:MM` と `▌` 無しの
全行、routine マーカー行を落とす規則は、新しい Requirement がそのまま引き継ぐ。消えるのは畳んだときの
`▌AI  HH:MM  <要約>  (+<n> 行)` の 1 行表示と、それを作る要約の規則である。`x` は詳細画面でも
何もしないキーになり、フッタのヒント（`card-detail`「詳細の本文領域はスクロールし、ヘッダ領域は固定する」）と
ヘルプの一覧（`help-screen`「ヘルプ画面は実装済みのキーだけを一覧する」）から `x` の行が消える。
キュー画面のプレビューと review thread 内のコメントは元から全文なので、見え方は変わらない。
