...
- `queue-screen`: `c` が「何もしないキー」から「主体の close の確認画面を開く」に変わる。フッタのヒントが 90 列から
  99 列になり、右のステータスの出所に close の中止・実行が加わる。ヒントの幅が変わって Scenario
  「幅 90 では取得中でもヒントとスピナーが両方出る」が成り立たなくなるので、フッタの Requirement は REMOVED + ADDED で
  作り直す（Scenario 名に幅が入っており MODIFIED では書き直せない）
- `card-detail`: 画面の状態が 9 個から 10 個（close の確認画面を追加）になり、詳細画面の `c` の対象が決まる。
  カード詳細・PR 詳細のフッタのヒントに `c close` が入る（144 列 / `PRs` 空で 96 列 / PR 詳細 109 列）
...
- フッタの budget の食い違い: `s15-new-issue` は「キュー画面のヒントは 80 列で埋まっているので `n 新規` は足さず、
  ...
  `c close` を足して 99 列にするので、両方が入ると「`c` はヒントにあって `n` は無い」状態になる。判断は人のもので、
  この change はそれに従う
...
- **`c` は現状どこにも割り当てが無い**。`internal/ui` に `case "c"` / `key == "c"` は 1 つも無く（`internal/ui/model.go`、
  ...
  既存テストで `c` を押すものも無く、壊れるのはヘルプの行数を見ているテストとフッタの幅を見ているテストだけ
...
- **フッタのヒントに `c close` を足す**（キュー 99 列 / カード詳細 144 列・`PRs` 空で 96 列 / PR 詳細 109 列。
  列数は tasks 1.0 の写し直しの時点、すなわち `L`（s28）と `n`（s15）が入った後のヒントで数え直した値）。
  人の回答（PR #10 のコメント）で決まった。位置は `m merge` の次で、`PRs` が空のカード詳細は `m merge` を出さないので
  `L ラベル` の次に置く（3 画面で `c close` の位置がそろう）。幅 99〜107 の端末では取得中や書き込み中にヒントが
  丸ごと消えるようになる（s14 がフッタを 80 列に伸ばしたときの「キーを隠すよりキーを出すことを採った」と同じ引き換え）
...
- **すでに closed の対象を close したときの表示は未実測のまま。** `gh issue close` / `gh pr close` がその場合に終了コード 0 を
  返すなら、TUI は `… を close しました` と出す。propose の環境にも apply の環境にも `gh` が無く確認できなかったので、
  spec は `gh` の結果をそのまま出すとだけ定めた。状態は変わらないので影響は表示だけである
