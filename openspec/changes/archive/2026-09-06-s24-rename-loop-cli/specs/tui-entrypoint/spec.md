...
- FROM: `### Requirement: sugi-loop バイナリが起動して 1 フレーム描画する`
- TO: `### Requirement: loop-cli バイナリが起動して 1 フレーム描画する`
...
描画内容にはアプリ名 `loop-cli` と、`q` で終了できることを示すフッタのヒントを含める。

#### Scenario: 初期フレームにアプリ名と終了案内が出る
- **WHEN** `ui.New` で作った Model の初期状態から描画文字列を得る
- **THEN** 文字列に `loop-cli` と `q 終了` が含まれる
