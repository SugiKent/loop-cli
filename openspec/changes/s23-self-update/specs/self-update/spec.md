...
`sugi-loop version` は、標準出力に `sugi-loop <現在の版>` の 1 行を MUST 書き、終了コード 0 で終わる。ネットワークを使わず、最新の版は調べない。
...
`sugi-loop update` は次の順で MUST 動く。
...
- **THEN** `go install github.com/SugiKent/loop-cli/cmd/sugi-loop@latest` が実行され、標準出力に `更新しました: v0.0.0-20260901000000-aaaaaaaaaaaa → v0.0.0-20260905143706-3ce8a526b4ec` が出て、終了コードは 0 である
