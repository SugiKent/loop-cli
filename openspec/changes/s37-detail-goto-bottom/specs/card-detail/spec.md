...
本文領域は Bubbles の viewport で描き、`j` / `↓` で 1 行下へ、`k` / `↑` で 1 行上へ、`PgDn` / `PgUp` で本文領域の高さぶん動き、`G`（Shift+G）で本文の末尾まで一度に動く（スクロールキーは mvp.md に無く、design.md 未決事項の既定値）。本文が本文領域の高さに収まっているときも `G` を受け取り、そのときはスクロール位置が動かない。`G` はカード詳細画面と PR 詳細画面でだけ動き、他の画面では何もしない。先頭と末尾で止まる。キュー画面から開いたとき、および PR 詳細とカード詳細を行き来したとき、スクロール位置は先頭に戻る。右ペインは独立したスクロールを持たず、スクロールキーは 2 ペインのときも常に左ペインの本文領域だけを動かす。
...
- **WHEN** `Body` が `行01` から `行60` までの 60 段落（各段落を空行で区切る）である issue の Card の詳細を、幅 100・高さ 20 の `Model` で開き、`G` を与えて `View` から ANSI エスケープを除いて読む
- **THEN** ヘッダ行 `org/app #<n>` を含み、`行60` を含み、`行01` を含まない

#### Scenario: G で PR 詳細のコメントの末尾が出る
...
#### Scenario: 本文が収まっているときの G はスクロール位置を動かさない
- **WHEN** `Body` が `行01` の 1 行である issue の Card の詳細を、幅 100・高さ 40 の `Model` で開き、`G` を与える前と後で `View` から ANSI エスケープを除いて読む
- **THEN** 2 つの `View` は等しく、どちらも `行01` を含む

#### Scenario: G の後にカード詳細へ戻るとスクロール位置は先頭に戻る
- **WHEN** 60 段落の `Body` と open PR 1 件を持つ Card のカード詳細を幅 100・高さ 20 で開き、`Enter` で PR 詳細に移って `G` を与え、`Esc` でカード詳細に戻って `View` から ANSI エスケープを除いて読む
- **THEN** `行01` を含む
