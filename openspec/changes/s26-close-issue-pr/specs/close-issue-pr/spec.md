...
タイトル・ラベルを決め（PR なら `State` も保持する）、表示名（issue は `<Repo> #<n>`、PR は `<Repo> PR#<n>`。`card-detail` の
詳細ヘッダと同じ表記）を作り、`gh` を呼ばずに close の確認画面へ移る。戻り先として `c` を押した画面を保持する。

- キュー画面: 選択行の主体（`queue-screen`「行の主体は `Card.Result` を出した Issue または PR」）。issue でも PR でもよい。
  選択行が無いときは何もしない
- カード詳細画面: その Card の `Issue`
- PR 詳細画面: 詳細の対象の PR
- close の確認画面・merge の確認画面・回答の確認画面・ヘルプ画面・URL 一覧画面: 何もしない

`c` のヒントは 3 画面のフッタには出さず、`?` のヘルプ（`help-screen`「ヘルプ画面は実装済みのキーだけを一覧する」）に出す。
キュー画面のフッタは既に表示幅 80 列で、幅 80 の端末では取得中や書き込み中にヒント全体が消えるため、ここではキーを増やさない。
...
`c` の押下では `gh` を呼ばない（対象の状態を取り直さない。理由と申し送りは design.md）。
...
- **WHEN** 同じ `Model` で主体が issue 140 であるバックログタブの行を選んで `c` を与え、`View` から ANSI エスケープを除いて読む
...
4. 拒否（`close-action`「`CheckClose` は open でない PR を拒否として返す」の結果）を 1 行ずつ `close できません: <理由>` の形で（赤）

本文は出さない。行が残りの高さに収まらない分は切り（スクロールしない。`answer-question` と `merge-pr` の確認画面と同じ）、
フッタは常に出す。フッタの左は拒否が空なら `y close  Esc 中止  q 終了`、空でなければ `Esc 中止  q 終了`。右は `queue-screen` と同じステータスとする。

確認画面のキーは MUST 次のとおり。

- `y`: 拒否が空のときだけ、close のコマンド（Requirement「close の結果をステータスに出し、再取得しない」）を返し、戻り先の画面に戻る。
  拒否が空でなければ何もしない
...
- **THEN** `close の確認: org/app #140` とその行の issue タイトル、`種別: issue`、`labels:` の行がこの順で含まれ、
  フッタに `y close`、`Esc 中止`、`q 終了` が含まれ、`close できません:` の行は無い

#### Scenario: PR の確認画面
- **WHEN** 同じ `Model` で今やるタブの PR 131 の行で `c` を与え、`View` から ANSI エスケープを除いて読む
- **THEN** `close の確認: org/app PR#131`、`種別: PR`、`labels: propose question` の行が含まれ、フッタに `y close` が含まれる

#### Scenario: すでに closed の PR には y が出ない
- **WHEN** `PRs` が `State` が `CLOSED` の PR 1 件だけの Card（`Issue` は nil）を今やるタブに持つ `Model` に `c` を与え、
  `View` から ANSI エスケープを除いて読む
- **THEN** `close できません: CLOSED の PR です` の行があり、フッタに `Esc 中止` と `q 終了` はあって `y close` は無い

#### Scenario: 拒否のある確認画面で y は何もしない
- **WHEN** 同じ確認画面の `Model` に `y` を与える
- **THEN** コマンドは返らず、画面は確認画面のままで、`Fake.Calls` に `ClosePR` は無い

#### Scenario: Esc で中止する
- **WHEN** Scenario「PR の確認画面」の `Model` に `Esc` を与え、`View` から ANSI エスケープを除いて読む
...
- **WHEN** Scenario「PR の確認画面」の `Model` に `a`、`t`、`m`、`c`、`o`、`?`、`u` を 1 つずつ与える
...
- **WHEN** Scenario「PR の確認画面」の `Model` に `y` を与え、返ったコマンドを実行して得たメッセージを `Update` に渡し、
  `View` から ANSI エスケープを除いて読む
...
- **THEN** `Fake.Calls` に `Method` が `CloseIssue`、`Number` が 140 の要素がちょうど 1 件ある

#### Scenario: close 中は close 中の表示で t と a と m と c は効かない
- **WHEN** 同じ手順で `y` を与えた直後（close のコマンドを実行する前）に `View` を読み、続けて `t`、`a`、`m`、`c` を 1 つずつ与える
...
- **WHEN** `y` を与えて close のコマンドが返った直後に `o` を与える
- **THEN** コマンドが返る
