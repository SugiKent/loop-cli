...
**Reason**: フッタのヒントに `c close` が入って表示幅が 90 列から 99 列になり、この Requirement の Scenario
「幅 90 では取得中でもヒントとスピナーが両方出る」が、名前の幅（90）とも検証する幅（100）とも合わなくなる。
Scenario 名が幅を含むため MODIFIED では書き直せないので、Requirement 名を
...
幅の Scenario を 99 列に合わせて 2 本（幅 108 で両方出る / 幅 107 はステータスだけ）に置き換える。
