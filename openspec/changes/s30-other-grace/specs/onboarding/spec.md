...
`Marshal` は mvp.md「設定ファイル」の例と同じ形式で、キーを `repos` / `refresh_interval_sec` / `merge_method` / `editor` / `notify` の順に、`repos` の各要素を `  - owner/name`（2 スペース + `- `）で、`refresh_interval_sec` を常に `120` で書く。コメントは書かない。`editor` の値は入力どおりに書く（`$EDITOR` は `$EDITOR` のまま。`config.Load` が `os.ExpandEnv` で展開する）。フォームで聞かない `other_grace_min` は書かず、`config.Load` の既定（30）に任せる。
