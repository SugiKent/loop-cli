...
`[3]進行中` のタブでは、種別の列に**段階ラベル名**（`todo` / `propose` / `apply` / `archive`、issue-label-driven のリポジトリなら `To Do` / `In Progress`）を GitHub のラベル色で出します。段階ラベルの無い issue / PR — 作った直後でラベルがまだ無い PR など — は `-`。表はリポジトリごとに `── org/app ── 2 件 ───` の見出し行で区切られ、行はリポジトリ順 → 段階順に並びます。
