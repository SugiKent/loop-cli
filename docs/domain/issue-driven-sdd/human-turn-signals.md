...
人が判断できないためである。worker が反映を終えた印はあるが checks が pending で判定表のどの行にも当たらない PR も、
この猶予に入って動いた直後の 30 分は進行中タブに出る。誰も触らないまま猶予を超えたものだけが人の出番として浮上する。`other_grace_min: 0` で猶予なし。
...
- `question` も `ai-assess:requested` も付いていない open PR で最新コメントが人のもの。auto-fix が受け取り中。
  ...
  ただし **worker が反映を終えた印がある PR は除き、判定表へ流す**。印は「本文 1 行目が `未確定の判断: 0 件`」と
  「`updatedAt` が最新コメントの `createdAt` より後（人のコメントの後に PR 自身が動いた）」の 2 つが揃うこと。
  worker は反映し終えたことを本文 1 行目と `question` を外すことで表し、そのたびにコメントを返すとは限らないので、
  コメントの順番だけで進行中に落とすと、同じ本文 1 行目を読む行 C と判断が食い違う。1 行目が `未確定の判断:` の行でない
  PR（外部から来た PR、`docs` PR）は反映し終えたと宣言していないので印は無い。`question` が付いている PR にこの除外は
  適用しない（付いているあいだ worker は反映を終えていない）。
...
- 上の PR の規則（最新コメントが人・AI 評価待ち）は、PR が最後に動いてから（`updatedAt`）3 時間以内に限る。どちらも AI が次に動く前提で
  ...
  worker が反映を終えた印がある PR はそもそもこの規則の対象でないので、時間切れの「その他」にもならない。3 時間を待たずに
  判定表へ流れ、どの行にも当たらなければ「その他」に「どの局面にも当たらない」と出る。
...
- 同一イベントから複数セッションが起動し、同趣旨のコメントが 3 連続で付くことがある。
  → 詳細画面ではコメントを全文で出し、`blocked-by:` 行を含む**最新**コメントだけを正本として上部に要約表示する
  （s38 で折りたたみを廃止した。同趣旨の重複は上部の要約 1 本で見分ける）。
...
| 日時 | 変更内容 | 理由 |
| --- | --- | --- |
| 2026-09-12-0330 | PR の進行中の規則（最新コメントが人）から、worker が反映を終えた印（本文 1 行目 `未確定の判断: 0 件` と、最新コメントより後の `updatedAt`）がある PR を外し、判定表へ流すことにした。時間切れの「その他」にも落とさない | worker が本文とラベルだけを更新してコメントを返さなかった PR が 3 時間今やるタブから消え、同じ本文 1 行目を読む行 C と判断が食い違っていたため（s36-classify-reflected-pr・#46） |
