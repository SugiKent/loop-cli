...
プラグインの規約（`routine-common` / `references/worker.md` / `routine-dispatch`。同期点は上流 `d8db3842`）から導いた、
...
| C | **merge する** | open PR、`propose` / `apply` / `archive` ラベル、本文 1 行目が `未確定の判断: 0 件`、`question` なし、checks 緑、mergeable | `gh pr merge`（確認ダイアログ必須） | 3 |
...
1. **1 操作 1 ラベル。** `--add-label` / `--remove-label` は 1 ラベルずつ別プロセスで呼ぶ。ラベル集合の置換は絶対にしない。
   ラベル書き込みは Routine を起動させ、dispatcher は付与イベントの時刻で判定するため。
...
5. **merge ガード。** `question` あり、または 1 行目 `未確定の判断: N 件` で N > 0、または checks 失敗、または `isDraft` なら merge を拒否し理由を表示する。
