...
| `c` | issue / PR を close する（対象は画面が見せているもの。確認 → `y` で実行、`Esc` で中止） | `gh issue close -R …` / `gh pr close -R …`。理由もコメントも渡さず、ラベルは触らない |
| `n` | 新規 Issue 作成。選択中の対象の repo に `$EDITOR` で作成（下書きに入っているタイトルと概要のプレースホルダー行が区切り。挟まれた部分がタイトル、概要より下が本文で、プレースホルダー行自体は issue に入らない）→ 確認 → 作成。repo は選ばせない | `gh issue create -R <選択中の repo>`。ラベルは付けない（段階ラベルも assignee も付けない） |
...
| 日時 | 変更内容 | 理由 |
| --- | --- | --- |
| 2026-09-10-1700 | キーバインド表に `c`（issue / PR を close する。確認あり）を追加 | もう要らないと判断した issue / PR を閉じる手立てが TUI に無く、`o` でブラウザへ出るしか道が無かったため（#4・s26-close-issue-pr） |
| 2026-09-10-1400 | キーバインド表の `n` を、タイトルと概要のプレースホルダー行で分ける形に変更 | エディタが空で開くと、どこに何を書けばよいかが分からなかったため。案内の 2 行そのものを区切りに使うので、案内は issue に入らない（#17・s29-new-issue-placeholders） |
