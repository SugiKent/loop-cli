...
運用方式は設定に書かない。取得のたびにリポジトリのラベル一覧を読み、`stage:todo` があれば issue-driven-sdd
（`stage:*` + `wip` + PR の段階ラベル）、無くて `To Do` があれば issue-label-driven（`To Do` / `In Progress` /
`Done` の 3 ラベル）と判定する（D-005）。GitHub 側でラベルを足せば、次の更新で方式が変わる（再起動は要らない）。
2 方式のリポジトリは 1 本のキューに混ざり、同じ 4 タブ・同じキー操作・同じ優先度で捌ける。
`repos` の要素に `mode:` が残っていると、削除を促すエラーで起動しない。
...
- キューが 0 件（全タブ空）のときは、画面中央に「`stage:*` / `To Do` ラベルの無いリポジトリは何も出ません。issue-driven-sdd の `routines-setup` を回すか、issue-label-driven の `To Do` ラベルを作ってください」のヒントを出す（「前提と未決事項」の 1 項目目を利用者に見せる形）。判定の材料はラベル一覧なので、直し方もラベルを作ることになる。
...
- **`routines-setup` を回していないリポジトリは空**。`stage:*` ラベルが無いリポジトリには段階ラベルが 1 つも付かず、TUI には何も出ない。
  issue-label-driven のリポジトリは `To Do` ラベルがあれば `To Do` / `In Progress` / `Done` で分類される。どちらの規約も持たない
  リポジトリは方式を判定できず、issue が「着手を承認する」としてバックログに並ぶだけになる。最初の動作確認は稼働中のリポジトリ 1 件で行う。
  方式を判定できないリポジトリで `t` を押すと、ラベルは書かずにフッタへ理由を赤で出す（推測したラベルを書くと worker の起動がずれる）。
...
| 日時 | 変更内容 | 理由 |
| --- | --- | --- |
| 2026-09-11-1030 | 設定ファイルから `mode` を削除し、運用方式をラベル一覧から判定する形に書き換え、0 件ヒントの文言を更新 | 方式を手で書かせる限り書き忘れという状態が残り、間違った承認ラベルを書く経路が閉じないため（#24） |
