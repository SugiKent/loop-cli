...
## D-005 運用方式はリポジトリのラベル一覧から取得のたびに判定する

決定日時: 2026-09-11-1030

リポジトリの運用方式（issue-driven-sdd か issue-label-driven か）の正本を、設定ファイルの
`repos[].mode` からリポジトリのラベル一覧に移す。`fetch.Fetch` が取得のたびにリポジトリごとに
`gh label list` を実行し、`stage:todo` があれば `sdd`、無くて `To Do` があれば `label` と判定する。
判定できたものだけを `fetch.Result.Modes` に入れ、`internal/ui` はそれを取得のたびに丸ごと差し替える。

s26-issue-label-driven の「方式は設定で持ち、ラベル集合からは判定しない」を覆す。設定に書かせる限り
「書き忘れたリポジトリに間違った承認ラベルを書く」経路が閉じないためである。当時の 3 つの懸念への答え:

| 当時の懸念 | 答え |
| --- | --- |
| `gh` 呼び出しが増える | 1 回の更新でリポジトリ数ぶん増えるが、`gh label list` は search のレート枠を使わず、詳細取得と同じ並行プールで走る |
| 両方式のラベルを持つリポジトリで判定が曖昧になる | `stage:todo` を先に見て `sdd` に倒す。`To Do` は Projects のカンバンの名前として sdd のリポジトリにも付くが、`stage:todo` は `routines-setup` が作るラベルで ILD 側が作る理由が無い |
| スナップショットに古い方式が残り、設定と食い違ったラベルを書く | 方式をスナップショットにも設定にも保存しない。方式が分からない間（初回取得の前・`ListLabels` の失敗・どちらのラベルも無い）は `t` を拒否してフッタに理由を出す |

分類と表示は方式が分からなくてもゼロ値（`sdd`）で行う。`Card.Result` は必ず 1 つ決まらないとカードが
並ばないためで、推測を許さないのは書き込み（`t`）だけである。`gh label list` の `--limit` は 1000 にする
（100 で切れると名前昇順で後ろにある `stage:todo` が落ち、`label` と誤判定して `To Do` を書きに行く）。

あわせて issue-label-driven の open PR は全件を `[1]今やる` に出す。`sdd` では auto-fix や worker が同じ PR を
受け取って進めるので「最新コメントが人 → 進行中」に落とす意味があるが、ILD は 1 issue = 1 PR を人が捌く方式で、
キューから外れると人の出番が見えなくなる。行 C と行 D の対象から `Closes #n` の絞り込みも外す。
...
| 日時 | 変更内容 | 理由 |
| --- | --- | --- |
| 2026-09-11-1030 | D-005 を追加 | 運用方式の正本を設定からリポジトリのラベル一覧へ移し、書き忘れという状態を無くすため（s30-mode-from-labels） |
