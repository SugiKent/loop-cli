...
- [ ] (P1) 今やるキュー画面: 4 タブ（今やる / バックログ / 進行中 / 異常）+ 選択行プレビュー。端末サイズによらず常に 2 ペイン
- [ ] (P1) 初回起動 onboarding: config.yml が無ければフォームで作る / gh 未認証の案内 / 空キューのヒント
...
| 日時 | 変更内容 | 理由 |
| --- | --- | --- |
| 2026-09-06-1200 | §3 に (P1) 自己更新を追加 | `go install` で配る以上、更新に気づく手立てと入れ直す手順が要るため（s23-self-update） |
| 2026-09-05-2130 | §3 に (P1) 初回起動 onboarding を追加 | 利用者要望。最初の起動は設定ファイルが無い状態から始まるため |
